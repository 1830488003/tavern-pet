...
(function () {
    ...
    function performScrape(doc) {
        ...
        albumItems.forEach((item) => {
            ...
            const title = titleElement
                ? titleElement.innerText.trim().replace(/[\\/:*?"<>|]/g, '_')
                : '未命名';

            const images = item.querySelectorAll(
                '.content img, .album_pic img',
            );
            const imageUrls = [];
            images.forEach((img) => {
        ...
        if (iframes.length > 0) {
            for (let i = 0; i < iframes.length; i++) {
                try {
                    ...
                    console.warn(
                        `无法访问第 ${i + 1} 个 iframe 的内容，可能是跨域限制。`,
                    );
                }
            }
        } else {
    ...
    if (results && results.length > 0) {
        ...
        console.log(
            '执行了所有检查（包括iframe），但仍然未能提取到任何表情包。请确保页面已完全加载，或尝试手动滚动页面到底部再运行脚本。',
        );
    }
})();
